  ...
  tokenquery.exec(function(tokenErr, tokens) {
    if (tokenErr) {
      ...
      if(tokens.length > 0) {
        ...
        query.exec(function(userErr, result) {
          if (userErr) {
            callback('Internal Server Error');
          }
          else {
            if ( result.length > 0 ) {
              ...
              convertParams.convertParamsToUser(params, function(paramErr, user) {
                if (paramErr) {
                  callback(paramErr);
                } else {
                  user.save(function(saveErr, returnedUser) {
                    if (saveErr) {
                      callback('Internal Server Error');
                    } else {
                      callback(null, {
        ...
        console.log(tokens);
        callback('Could not create user. Invalid token.');
      }
    }
  });
};
