...
exports.validateRequiredParams = function(params) {  
	...
	if(!params.hasOwnProperty('role')) {
		...
		if(!isNonEmptyStr(params.role) && validRole(params.role)) {
			params = sanitize(params);
			params.err = { err: 'Role must be non-empty string and either "admin" or "editor".'};
			return params;
		}
	}
	...
	if(!params.hasOwnProperty('email')) {
		...
		if(!EMAIL_REGEXP.test(params.email)) {
			params = sanitize(params);
			params.err = { err: 'Invalid Email address.'};
			return params;
		}
	}
	...
	if(!params.hasOwnProperty('password')) {
		...
		if(!isNonEmptyStr(params.password)) {
			params = sanitize(params);
			params.err = { err: 'Password must be non-empty string'};
			return params;
		}
	}
